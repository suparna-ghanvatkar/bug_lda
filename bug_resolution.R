...
library(stringr)
drv=dbDriver("MySQL")
con=dbConnect(drv,dbname='bug',user='root',password='rohit123')
...
tab<-data.frame(report_id=integer(),timefor_resolution=integer(),severity=character(),stringsAsFactors=FALSE)
i<-1
while(i<4617)
{
  tab[i,1]=table2[i,1]
...
i<-1
tab1<-data.frame(severity=character(),no_of_bug=integer(),average_time=numeric(),stringsAsFactors=FALSE)
